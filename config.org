...
*;;  Development
*;; * Encryption
S;; OPS is a CLI tool to encrypt/decrypt “secrets” inside files so they can be put inside version control safely. Here SOPS is an Emacs minor mode which makes operating on SOPS files easier and prevents corruption.
#+begin_src emacs-lisp;;
 ;;
  (use-package sops
    :hook
